...
use datafusion::physical_plan::joins::{CrossJoinExec, NestedLoopJoinExec, SortMergeJoinExec, StreamJoinPartitionMode, SymmetricHashJoinExec};
...
use crate::protobuf::{
    self, window_agg_exec_node, PhysicalPlanNode, PhysicalSortExprNodeCollection, SourceType,
};
        ...
        match plan {
            ...
            PhysicalPlanType::HashJoin(hashjoin) => {
                ...
                let on: Vec<(PhysicalExprRef, PhysicalExprRef)> = hashjoin
                    ...
                    .collect::<Result<_>>()?;
                let join_type = protobuf::JoinType::try_from(hashjoin.join_type).map_err(|_| {
                    proto_error(format!(
            ...
            PhysicalPlanType::SymmetricHashJoin(sym_join) => {
                let left = into_physical_plan(&sym_join.left, registry, runtime, extension_codec)?;
                let right =
                ...
                let on = sym_join
                    ...
                    .collect::<Result<_>>()?;
                let join_type = protobuf::JoinType::try_from(sym_join.join_type).map_err(|_| {
                    proto_error(format!(
            ...
            PhysicalPlanType::SqlScan(scan) => {
                let pt_conf = scan.config
                    .as_ref()
                    .ok_or_else(|| proto_error("Missing required field in protobuf"))?;

                let source_type = match SourceType::try_from(pt_conf.source_type).map_err(|_| {
                    proto_error(format!(
                        "Received a Scan message with unknown SourceType {}",
                        pt_conf.source_type
                    ))
                })? {
                    SourceType::Mysql => Ok(hexa::datasource::dispatch::SourceType::Mysql),
                    SourceType::Postgresql => Ok(hexa::datasource::dispatch::SourceType::Postgresql),
                    _ => Err(proto_error("Not support source type")),
                }?;

                let source_config = hexa::datasource::dispatch::DataSourceConfig {
                    name: pt_conf.name.clone(),
                    source_type,
                    host: pt_conf.host.clone(),
                    port: pt_conf.port,
                    username: pt_conf.username.clone(),
                    password: pt_conf.password.clone(),
                    database: Some(pt_conf.database.clone()),
                    option: pt_conf.option_parameters.clone(),
                };
                let schema: Arc<Schema> = Arc::new(convert_required!(scan.schema)?);

                let scan_config = hexa::execution::physical_plan::config::SourceScanConfig {
                    sql_list: scan.sql.clone(),
                    config: source_config,
                    source_schema: schema,
                };
                Ok(Arc::new(
                    hexa::execution::physical_plan::sql_scan::SqlScanExec::new(scan_config),
                ))
            }
            PhysicalPlanType::MergeJoin(merge_join) => {
                let left = into_physical_plan(&merge_join.left, registry, runtime, extension_codec)?;
                let right =
                    into_physical_plan(&merge_join.right, registry, runtime, extension_codec)?;
                let left_schema = left.schema();
                let right_schema = right.schema();
                let on = merge_join
                    .on
                    ...
                    .collect::<Result<_>>()?;
                let join_type = protobuf::JoinType::try_from(merge_join.join_type).map_err(|_| {
                    proto_error(format!(
                        "Received a SymmetricHashJoin message with unknown JoinType {}",
                        merge_join.join_type
                    ))
                })?;
                let filter = merge_join
                    .filter
                    .as_ref()
                    .map(|f| {
                        let schema = f
                ...
                let options = merge_join.sort_options.iter()
                    .map(|o|{
                        Ok(SortOptions {
                            descending: !o.asc,
        ...
        Self: Sized,
    {
        ...
        if let Some(exec) = plan.downcast_ref::<ExplainExec>() {
            return Ok(protobuf::PhysicalPlanNode {
                physical_plan_type: Some(PhysicalPlanType::Explain(protobuf::ExplainExecNode {
        ...
        if let Some(exec) = plan.downcast_ref::<AvroExec>() {
            return Ok(protobuf::PhysicalPlanNode {
                physical_plan_type: Some(PhysicalPlanType::AvroScan(protobuf::AvroScanExecNode {
                    base_conf: Some(exec.base_config().try_into()?),
                })),
            });
        }
            ...
            let expr = exec
                ...
                .map(|expr| {
                    ...
                    Ok(protobuf::PhysicalExprNode {
                        expr_type: Some(protobuf::physical_expr_node::ExprType::Sort(sort_expr)),
                    })
                })
                .collect::<Result<Vec<_>>>()?;
            return Ok(protobuf::PhysicalPlanNode {
        ...
        match extension_codec.try_encode(plan_clone.clone(), &mut buf) {
            Ok(_) => {
                let inputs: Vec<protobuf::PhysicalPlanNode> = plan_clone
                    .children()
                    .into_iter()
                    .map(|i| protobuf::PhysicalPlanNode::try_from_physical_plan(i, extension_codec))
                    .collect::<Result<_>>()?;
